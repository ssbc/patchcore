...
exports.create = function (api) {
  ...
  return nest({
    'about.obs': {
      ...
      imageUrl: (id) => computed(socialValue(id, 'image'), (blobData) => {
        if (!blobData) return fallbackImageUrl
        const blobId = typeof blobData.link === 'string' ? blobData.link : blobData
  ...
  function valueFrom (id, key, author) {
    ...
    return MutantPullValue(() => {
      return api.sbot.pull.stream((sbot) => sbot.about.valueFromStream({ dest: id, key, id: author }))
    })
  }
